...
int main() {
    ...
    int selected;
    largeNumber_128 q, p, e, N, phiN;
    
    //Xác định p và q
    cout << "----MENU----\n1. Generate random p, q\n2. Enter p, q\n";
    cin >> selected;
    switch (selected) {
    case 1:
        cout << "Random prime number q: ";
        q = createRandomPrime();
        cout << q << endl;

        cout << "Random prime number p: ";
        p = createRandomPrime();
        cout << p << endl;
        break;
    case 2:
        do {
            ...
            if (!primalityCheck_result(q)) {
                cout << "Invalid input. Please enter a prime number." << std::endl;
            }
        } while (!primalityCheck_result(q));

        do {
            cout << "Enter a prime number p: ";
            cin >> p;

            if (!primalityCheck_result(p)) {
                cout << "Invalid input. Please enter a prime number." << std::endl;
            }
        } while (!primalityCheck_result(p));
        break;
    }
    cout << "\n\nSelected q = " << q << "\nSelected p = " << p;
    ...
    switch (selected) {
    case 1:
        cout << "Random prime number e: ";
        e = createRandomPrime_e(phiN);
        cout << e << endl;
        break;
    case 2:
        do {
            cout << "Enter a prime number e: ";
            cin >> e;

            if (gcd(e, phiN) != 1) {
                cout << "Invalid input. Please enter a prime number again." << std::endl;
            }
        } while (!primalityCheck_result(e));
        break;
    }
    cout << "\n\nSelected e (Public Key) = " << e;

    // Tính d
    largeNumber_128 d;
    d = calculateD(e, phiN);
    cout << "\nSelected d (Private Key) = " << d << endl;


    // Hiển thị khóa công khai và khóa riêng tư
    ...
    case 1: // Mã hóa
        ...
        EncryptStringOrNumber(plainText, e, N);
        break;
    case 2: // Giải mã
        cin.ignore();
